...
class SepticTankController extends Controller
{
    ...
    public function listCleanedBooking(Request $req)
    {
        ...
        try {
            ...
            // $list = $list->whereBetween('stb.driver_delivery_update_date_time', [$req->fromDate, $req->toDate])->values();
                ...
                if ($req->fromDate != null) {
                    $list = $list->filter(function ($item) use ($req) {
                        $date = substr($item->driver_delivery_update_date_time, 0, 10);
                        return $date >= $req->fromDate && $date <= $req->toDate;
                    })->values();
                }
            $ulb = $this->_ulbs;
    ...
    public function searchApp(Request $req)
    {
        try {
            ...
            $bookings = StBooking::select('applicant_name', 'booking_date', 'booking_no', 'cleaning_date', 'payment_status','assign_date' ,'feedback','id');
            $cancellations = StCancelledBooking::select('applicant_name', 'booking_date', 'booking_no', 'cleaning_date', 'payment_status','assign_date', 'feedback','id');
